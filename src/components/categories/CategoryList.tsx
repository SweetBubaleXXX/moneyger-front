...
export const CategoryList = ({ filter, reorder, loading, onSubmitReorder, onItemClick, sx }: CategoryListProps) => {
  ...
  useEffect(() => {
    if (categories.data && !isLoading) {
      setOrderedCategories(categories.data.sort((a, b) => a.displayOrder - b.displayOrder));
    }
  }, [categories.data, isLoading]);

  return (
    <Box maxWidth={450} px={2} mx="auto" sx={sx}>
      <Reorder.Group as="div" axis="y" values={orderedCategories} onReorder={setOrderedCategories}>
