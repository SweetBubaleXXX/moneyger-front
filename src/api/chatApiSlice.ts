  ...
  endpoints: (builder) => ({
    getMessages: builder.query<EntityState<Message>, void>({
      ...
      transformResponse: (response: Message[]) =>
        messagesAdapter.addMany(messagesAdapter.getInitialState(), camelcaseKeys(response)),
      onCacheEntryAdded: async (_, { updateCachedData, cacheDataLoaded, cacheEntryRemoved, getState }) => {
